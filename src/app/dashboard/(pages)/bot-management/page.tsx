...
import { useGetPhones } from "../../hooks/phone-hooks";
import { BotManagementAvailableCard } from "./components/bot-management-available-card";
import { BotManagementDisabledCard } from "./components/bot-management-disabled-card";
import { BotManagementSkeletonLoader } from "./components/bot-management-loader";
import { BotManagementPagination } from "./components/bot-management-pagination";
import { BotStatsCard } from "./components/bot-management-stats-card";
...
const PhoneDashboard: React.FC = () => {
  ...
  const availablePhones = phones.filter(
    (phone) => phone.status === "available" || phone.status === "in_call"
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">
                Phone Extensions Dashboard
              </h1>
              <p className="text-muted-foreground">
                Real-time call center phone monitoring
              </p>
            </div>
          </div>
            ...
            <Select
              value={
                filters.server_id !== undefined
                  ? filters.server_id.toString()
                  : ""
              } // Default to "" when filters.server_id is undefined
              onValueChange={(value) =>
                setFilters((prev) => ({
                  ...prev,
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              ...
              <BotStatsCard
                ...
                value={`${
                  phones.length > 0
                    ? Math.round((availablePhones.length / phones.length) * 100)
                    : 0
                }%`}
